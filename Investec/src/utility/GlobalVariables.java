package utility;

public class GlobalVariables {
	 public static final String AUT_BROWSER = "Chrome";
	    
	    public static final String Actimize_URL = "https://www.investec.com/";

	    public static final String DRIVERDATA = "webdriver.chrome.driver";
	    public static final String DATALOCATION = "src/Chromedriver/chromedriver.exe";
	    public static final String PATH_TESTDATA = "C:\\Selenium Scripts\\BPM_HealthChecks\\";


	    public static final String FILE_TESTDATA ="";

	    public static final String TESTDATA = "PASS";

	    public static final String rowNum = "1";
	    public static final String colNum = "1";





	    //NOT USED
	    public static final int AUT_MAX_WAIT = 60;	//max wait time for object load




}
